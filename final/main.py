...
def main(request):
    ...
    """
    data = main_process(from_date=(datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d'),to_date=(datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d'))
    data.to_gbq(destination_table='dataEngineeringTaskv1.wikipediaTopPages', if_exists="append")
